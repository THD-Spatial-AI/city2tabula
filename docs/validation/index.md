...
!!! info "Direct match only"
    Only surfaces with a property row whose `feature_id` matches the surface's own ID are validated. Parent-level properties (for example slope stored on the building rather than on each roof surface) are not inherited. See [Known Limitations](#known-limitations).
...
??? tip "Discovering property names in a dataset"
    ```sql
...
Full-dataset runs (May 2026), with no sample limit: every matched surface was validated.
...
!!! tip "The geometry argument"
    Wall RMSE for invalid/non-planar surfaces is **2,431×** higher than valid+planar in Deggendorf, and **6,096×** higher in Vienna. For the valid and planar subset alone, wall area RMSE drops to 0.003 to 0.016 m², which places the elevated overall RMSE in the source geometry rather than the extraction pipeline.
...
| ------- | --------- | ------------ | ----------------- | ----------------- | ---------- | ---------- |
| Deggendorf (DE) | Min height (m) | 2.148 | 2.148 | n/a | 125,991 | 0 |
| Deggendorf (DE) | Max height (m) | 3.514 | 3.514 | n/a | 125,991 | 0 |
| Freiburg (DE) | Min height (m) | 3.820 | 3.820 | n/a | 110,163 | 0 |
| Freiburg (DE) | Max height (m) | 4.451 | 4.451 | n/a | 110,163 | 0 |
| Vienna (AT) | Min height (m) | 11.813 | 11.813 | n/a | 106,572 | 0 |
| Vienna (AT) | Max height (m) | 12.331 | 12.331 | n/a | 106,572 | 0 |
...
Deggendorf (Bavaria) achieves the lowest RMSE on roof tilt (0.038°) and roof area (0.042 m²) because tilt is stored at surface level, so the feature ID match is direct. Freiburg's tilt RMSE (1.240°) is higher because Baden-Württemberg exports `Dachneigung` at the BuildingPart level; validation can only match ~64% of roof surfaces, and those matches carry more noise from the parent-to-surface propagation.
...
The larger Vienna height RMSE (12.3 m against 3.5 to 4.5 m for Germany) is consistent with Vienna's LoD2.1 geometry being structurally more complex: more surfaces per building means more opportunities for mis-assignment.
...
1. Install City2TABULA, following [Setup and Installation](../installation/setup.md).
2. Download the source datasets from the links in the [Datasets](#datasets) table.
3. Import: `./c2t -create-db`.
4. Extract: `./c2t -extract-features`.
5. Set `configs/config_{country}.yaml` and `.env` for the dataset.
